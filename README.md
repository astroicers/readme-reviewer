...
> LLM 讀 README 判**第一屏定位 / 最短可執行路徑 / 寫作品質 / 限制誠實**
...
| **R-004** | 限制誠實 | 說了什麼時候**不該**用嗎？易腐內容有時效標記嗎？ |
...
| **craft verdict** | **信「有沒有問題」，不信刻度** | 主判，但**觸發率尚未實測**——見下 |

> ⚠️ **`≥2 mixed → needs-revision` 這個門檻是借來的，本專案還沒有自己的觸發率實測。**
>
...
>
> 本專案第一天就讓 `mixed` 計費，但**那個數字還沒有在真實 README 上驗過**。
> 若你發現它過度觸發，**先懷疑 R-004**（借自該專案信度最低的維度，κ=0.400），
> 再考慮動門檻。
