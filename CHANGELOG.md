...
斷言(掛 CI,R1 基線 import 自 blind_agreement 不重抄);判讀逐字紀錄在
`reviews/blind-2026-09-02-r2/`(列入禁讀清單)。

- **預先登記 7/8 三方一致命中**(批次 #2 報告的推導表寫於開跑前):
  sindre R-004 三值 → 一致 n/a、public-apis R-005 3:1 → 一致 mixed(anti-pattern
  落點修好)、dbt R-002 → 一致 poor、fCC 形狀 → 一致 hosted
- **R-004 兩兩一致 69.4 → 77.8**;R-002 +5.6;verdict 三方全同 8→9;
  R-001/R-005 兩輪持平(100 / 94.4)
- ⚠️ **R-003 −8.3**:修 fCC 的「artifact 禁反推」規則把 linux 打裂
  (形狀 2:1、R-003 三值全開)——修一個形狀弄壞另一個,同型第 N+2 次
- ⚠️ fiber R-004 從一致 good 被新量詞打成 2:1(「宣稱即陳述」新縫,預登記唯一未中);
  badge-only 易腐撞出 scope 與 equivalent_forms 互拉(批次 #2 同時寫的兩節)
- friction 收斂 8 條 → **批次 #3 蓄積,依裁定先不處理**

### ⭐ 裁定:開發驅動輪改為真實使用

兩輪盲判後整體一致度持平(86.1 → 87.2)、修一縫露一縫——逼近姊妹專案的已知邊界
(「再多量測也解析不出判準修訂的效果」)。**下一個有權觸發條文修訂的事件是
真實審查中的誤判**;實驗室盲判降為大改版後的回歸測試,不再當開發驅動。

---

## [0.3.0] — 2026-09-02(rubric 0.3.0 / 工具 0.3.0)

**第二次誤判批次處理:10 條(盲判 friction 收斂 9 + verbatim 補登 1)→ 9 修 / 1 刻意不修。**
純條文改動、零程式行為變更;逐條裁決與分裂格推導:`reviews/2026-09-02-misjudgment-batch-2.md`。
修法原則:**把三位盲判者被迫現場立法時收斂到的讀法寫進條文**,不是發明新規則。

### R-004(盲判實測兩兩 69.4%、vs 作者 47.2% 的失穩靶心)

- 新增 **`scope_of_perishable`**:判定對象是**載重宣稱**(讀者會據以行動的版本/API/
  定價/效能/統計);範例與設定樣張內的字面(`@v1` pin)、無數字的修辭形容、
  **外部連結本體**(鏈腐歸 H-005/R-005)皆不算
- 新增 **`statement_test`**:「陳述」須**對讀者的選擇有排除力**;單一形容詞與授權
  聲明不算;**清單型認收錄立場**(清單的限制就是它不收什麼)
- 序 1 明定:文內無載重宣稱時,第二條件**空缺為真**(有陳述即 good)——
  盲判者曾指出這格在 order 裡不存在
- 序 3 封頂 mixed **刻意保留**,rationale 寫進條文一行(機械同步只解決鮮度,
  沒回答「何時不該用」)
- equivalent_forms 補**自動更新 badge**(版本/CI badge 由服務端渲染)
- **R-004 與 R-005 的證據欄必記 decision_order 序號**(序 2/序 3 殊途同歸 mixed,
  不記序號路徑分歧在輸出層不可見;三位盲判者已自發照做,升為必填)

### R-005

- decision_order 插**序 2:可指認但命中 anti_pattern → mixed** —— 補上「anti-pattern
  清單與取值程序脫鉤」的落點(三位盲判者與作者判定分裂的成因,四方同指條文)
- 「已失效」明定**允許引用廣為人知的服務終止事實**(freenode 型),證據欄須具名;
  不確定不判 poor
- 主體「內文指認」認三形式:第一人稱組織聲音、品牌產品站、指向名冊的指標
  (MAINTAINERS file);URL 帳號名仍不算

### 其餘

- **craft_value_mapping**:mixed 限定「傷及**基礎要件**的瑕疵」;不傷基礎要件的
  記 findings 不降值(字面讀會讓 good 不可達——三位各自發明同型內規,現在是條文)
- **R-002 `delegation_stance`**:CLI **外鏈安裝文件不豁免**(README 內零指令=poor);
  library **豁免涵蓋安裝指令**;hosted 服務門面**消費/貢獻路徑擇一完整**
- **形狀表 + 「hosted 服務門面」列**;明寫**形狀由 artifact 主體決定,不由 README
  內容反推**(倒果為因是實測分歧源)
- **R-001 渲染近似規則**:HTML 頭部以「首個標題/logo 區塊+其後第一段散文」為第一屏,
  raw 行數不作數

### 驗證

- 零程式行為變更:selftest / evals / resimulate_18 / blind_agreement 原樣全綠
  (後兩者為 0.2.0 時代的歷史量測,EXPECT 凍結)
- 污染紀律:判準條文段經 grep 實檢**無語料 repo 名、無觸發統計**
  (freenode/travis-ci.org 為世界知識條款的允許例示,刻意保留;
  第一版曾寫入「getdbt.com 型」具名例,自查移除)
- ⚠️ 分裂格的「單一出口」推導是作者做的(已讀過盲判值,有污染)——
  **新條文是否真的收斂一致度,只有下一輪盲判能說**

### 同版稍早落地的來源查證與盲判資料(本版的觸發來源,PR #4)
